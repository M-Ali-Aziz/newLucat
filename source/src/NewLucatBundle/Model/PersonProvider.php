...
class PersonProvider extends AbstractProvider
{
    public function SynchronizeLucatPerson()
    {
        ...
        $parentId = $this->getParentId('newLucatPersonParentId');
        ...
        foreach ($result as $lucatOpenPerson) {
            ...
            if ($o_id) {
                // Update the Object
                $lucatPersonObj = DataObject\NewLucatPerson::getById($o_id);
                ...
                // Save new Object
                $lucatPersonObj = new DataObject\NewLucatPerson();
    ...
    private function getRoll($uid)
    {
        ...
        $rollObj = new DataObject\NewLucatRoll\Listing();
        $rollObj->setUnpublished(true);
        $rollObj->setCondition("uid = ?", $uid);

        foreach ($rollObj as $roll) {
            $rollArr[] = DataObject\NewLucatRoll::getById($roll->getId());
        }
    ...
    private function getOrganisation($rolls)
    {
        ...
        foreach ($departmentNrArr as $departmentNumber) {
            $orgObj = new DataObject\NewLucatOrganisation\Listing();
